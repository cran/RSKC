		    ...
		    miss<-FALSE
     		if(sparse) {
                        Result<-RSKC.a1.a2.b(d,L1,ncl,nstart,alpha,n,f,g,Nout,silent)
	                    	...
	                    	# non sparse
	                    	Result<-RSKC.trimkmeans(d,k=ncl,trim=alpha,scaling=FALSE,runs=nstart,points=NULL,maxit=10000)
		    ...
		    miss<-TRUE
                   if (sparse){
	                         Result<-RSKC.a1.a2.b.missing(d,L1,ncl,nstart,alpha,n,f,g,Nout,silent)
    	                  	 ...
    	                  	 # non sparse
    		                 Result<-RSKC.trimkmeans.missing(d,k=ncl,W,sumW,trim=alpha,scaling=FALSE,
    		                         runs=nstart,points=NULL,maxit=10000)
	                         if(Nout!=0) {
