...
for (i in 1:iteration){	
     ...
     reduced.f<-sum(W!=0)
	 A1<-RSKC.trimkmeans.missing(w.data,ncl,reduced.W,sumW,trim=alpha,runs=nstart,points=NULL,maxit=10000)
	 if (Nout!=0){out.a1<-which(A1$classification==(ncl+1))}else{out.a1<-n+1}
	 if (i!=1&oldss!=-Inf){
	       Ab4<-RSKC.trimkmeans.missing(w.data,ncl,reduced.W,sumW,trim=alpha,runs=1,
	                      points=cent.missing(w.data[-save.out.a1,,drop=FALSE],C[-save.out.a1],
	                      ncl,reduced.f),maxit=10000)
	       if(Nout!=0) out.a1.b4<-which(Ab4$classification==(ncl+1)) else out.a1.b4<-n+1
	       b4sum<-sum(Ab4$disttom[-out.a1.b4])
	       afsum<-sum(A1$disttom[-out.a1])
	...
	if(Nout!=0) new.C<-class.trimk.missing(w.data,w.mu,new.C,ncl,Nout,reduced.W,sumW)
